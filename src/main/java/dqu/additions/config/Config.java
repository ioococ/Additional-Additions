...
import dqu.additions.config.value.ListConfigValue;
...
public class Config {
    ...
    private static final String PATH = FMLPaths.CONFIGDIR.get().resolve("additions-config.json").toString();
    private static final File DBFILE = new File(PATH);
    public static boolean initialized = false;
    private static JsonObject db;

    private static String format(String message) {
        return String.format("[%s] %s", Additions.namespace, message);
    }
    ...
    public static void load() {
        ...
        try {
            ...
            Additions.LOGGER.error(format("Unable to load configuration file!"), e);
        }
        ...
        try {
            ...
            Additions.LOGGER.error(format("Configuration file is damaged!"), e);
    ...
    private static void save() {
        try {
            ...
            Additions.LOGGER.error(format("Unable to save configuration file!"), e);
        }
    }
    ...
    private static void convert(int version) {
        ...
        Additions.LOGGER.info(format("Converted outdated config."));
    }
    ...
    private static void repair() {
        ...
        if (repaired > 0) {
            Additions.LOGGER.info(format("Repaired " + repaired + " config properties"));
        }
